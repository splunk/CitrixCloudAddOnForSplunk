...
def get_new_records(logger, ew, inputItems, customerid, siteId, endpoint, token, proxyParam):
    ...
    for ep in endpoint.split("|"):
        ...
        if "{}" in ep: #if the endpoint has replacement characters in it, add the site id and remove for the sourcetype
            logger.info("one of the cvad_operational endpoints contains curls, replacing those")
            finalEP = ep.format(siteId)
            epSourceType = epSourceType.replace("{}", '')
        else:
            finalEP = ep
        ...
        while True:
            ...
            if itemsKey == None:
                ...
                recordEvent.sourceType = "citrix:cvad:operational:{}".format(epSourceType)
                recordEvent.done = True
                recordEvent.unbroken = True
                recordEvent.host = siteId
                recordEvent.time = None
                
                ew.write_event(recordEvent)

            else:
                logger.info("cvad_operational looks like it does have items in data, enumerating and writing")
...
class CVAD_OPERATIONAL(smi.Script):
    ...
    def get_scheme(self):
        scheme = smi.Scheme('cvad_operational')
        scheme.description = 'CVAD Operational'
    ...
    def stream_events(self, inputs, ew):
        ...
        get_new_records(logger, ew, inputItems, customerid, siteId, endpointData, token, proxyParam)
